...
class EnableIndexingCommand extends ContainerAwareCommand
{
    ...
    {
        $this
            ->setName('riak:bucket:indexing:enable')
            ->setDescription('Activate indexing on a given bucket')
            ...
            ->addOption(self::OPTION_NO_CHECK, "y", InputOption::VALUE_NONE,     "Automatically answer to all questions. Be aware that indexing will be activated without the option to stop the command.")
        ;
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if ((!$input->hasOption(self::OPTION_NO_CHECK) || !$input->getOption(self::OPTION_NO_CHECK)) && !$dialog->askConfirmation($output, '<question>Are you sure you want to activate indexing ? Last chance [y|n] : </question>', false)) {
            return 0;
        }
        ...
        $bucket->save();
        $output->writeln("<info>Indexing on '$bucketName' activated</info>");
